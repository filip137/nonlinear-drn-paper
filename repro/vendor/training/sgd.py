...
class EquilibriumProp(GradientEstimator):
    ...
    * if the variant is 'centered', then nudging_1 = - nudging and nudging_2 = + nudging
    ...
    def return_free_and_nudged_states(self):
        """Returns the free and nudged states of the layers

        This method is called to help find the correct size of the beta

        Returns:
            layers_free: list of Tensor of shape layer_shape and type float32. The free state of the layers
            layers_first: list of Tensor of shape layer_shape and type float32. The nudged state of the layers
        """

        # TODO: this implementation is likely suboptimal in the case of e.g. the Readout cost function
        cost_grads = [self._cost_fn._grad(param, mean=True) for param in self._cost_fn.params()]  # compute the direct gradient of C, if C explicitly depends on parameters
        all_layers = self._energy_minimizer._layers  # or the energy_fn.layers()
        ...
        return layers_free, layers_first
