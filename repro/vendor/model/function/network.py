...
class Network():
    ...
    def layers(self):
        """Return all layers in the underlying function."""
        return self._function.layers()
    ...
    def refresh_input(self):
        """Rebuild the input layer state from the last raw batch."""
        if hasattr(self._input_layer, "refresh_input"):
            self._input_layer.refresh_input()
            return
        if self._last_input_values is None:
            return
        self._input_layer.set_input(self._last_input_values)

    def save(self, path):
        """Proxy to save the underlying function parameters."""
        self._function.save(path)

    def load(self, path):
        """Proxy to load parameters into the underlying function."""
        self._function.load(path)
