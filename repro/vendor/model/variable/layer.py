...
def layer_index(layer_or_name):
    """Return a stable layer index.

    Prefer the per-network depth index when present; otherwise fall back to the
    integer suffix of names like ``Layer_10``.
    """

    if hasattr(layer_or_name, "_depth_index"):
        return int(layer_or_name._depth_index)

    if hasattr(layer_or_name, "name"):
        name = layer_or_name.name
    elif hasattr(layer_or_name, "_name"):
        name = layer_or_name._name
    else:
        name = str(layer_or_name)

    prefix, sep, suffix = name.rpartition("_")
    if sep != "_" or prefix != "Layer" or not suffix.isdigit():
        raise ValueError(f"Expected a layer name like 'Layer_<index>', got {name!r}")
    return int(suffix)
