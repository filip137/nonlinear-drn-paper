...
class Minimizer:
    ...
    def __init__(self, fn, updaters, num_iterations, mode,
        ...
        Args:
            ...
            voltage_amp (float, optional): voltage amplification factor; if None, inferred from `fn` when available
            current_amp (float, optional): current amplification factor; if None, inferred from `fn` when available
        """

        # Unwrap to underlying energy function if wrapped (e.g., AugmentedFunction)
        ...
        # Optional hardware scaling attributes (propagated for resistive networks)
    ...
    def voltage_amp(self):
        """Optional voltage amplification factor associated with the minimizer (if provided)."""
        return self._voltage_amp

    @property
    def current_amp(self):
        """Optional current amplification factor associated with the minimizer (if provided)."""
